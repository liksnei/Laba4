...
// Перо для рисования графика
        graphicsStroke = new BasicStroke(2.0f, BasicStroke.CAP_BUTT,
                BasicStroke.JOIN_ROUND, 10.0f, new float[]{40.0f, 10.0f, 10.0f, 10.0f, 20.0f,10.0f, 10.0f, 10.0f,40.0f,10.0f}, 10.0f);
...
// Отобразить график
    ...
    ////////////////////////////////////////////////////////////////
    // Метод для проверки, содержит ли число только чётные цифры
    private boolean isValueInRange(double value) {
        double intValue = Math.floor(value); // Округлить значение вниз до ближайшего целого числа
        double lowerBound = intValue - 0.1;
        double upperBound = intValue + 0.1;
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        for (Double[] point : graphicsData) {
            ...
            if (isValueInRange(point[1])) {
                // Если значение функции находится в диапазоне 0.1 от целого числа, сделать маркер синим

                canvas.setColor(Color.BLUE);
            } else {
                // В противном случае оставить красный цвет
                canvas.setColor(Color.RED);
            }
            ...
            // Определить точки для треугольника
            int[] xPoints = {
                    (int) center.getX(),
                    (int) (center.getX() - markerSize / 2),
                    (int) (center.getX() + markerSize / 2)
            };
            int[] yPoints = {
                    (int) (center.getY() - markerSize / 2),
                    (int) (center.getY() + markerSize / 2),
                    (int) (center.getY() + markerSize / 2)
            };

            // Установить точки треугольника
            markerTriangle.xpoints = xPoints;
            markerTriangle.ypoints = yPoints;
            markerTriangle.npoints = 3;

            // Нарисовать контур треугольника
            canvas.drawPolygon(markerTriangle);
        }
    }
